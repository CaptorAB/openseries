...
def plot_stochastic_processes(processes: list, title: str = None) -> (go.Figure, str):
    ...
    plot(
        figure, filename=plotfile, auto_open=True, link_text="", include_plotlyjs="cdn"
    )
...
def heston_construct_correlated_path(
    param: ModelParameters, brownian_motion_one: np.ndarray
...
def get_correlated_geometric_brownian_motions(
    param: ModelParameters, correlation_matrix, n: int
) -> list:
    """
