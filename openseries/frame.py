...
class OpenFrame(object):
    ...
    def value_ret(self) -> pd.Series:
        ...
        if self.tsdf.iloc[0].isin([0.0]).any():
            raise Exception(
                "Error in function value_ret due to an initial value being zero. ({})".format(
                    self.tsdf.head(3)
                )
            )
        else:
    ...
    def value_ret_func(
        ...
        if self.tsdf.iloc[0].isin([0.0]).any():
            raise Exception(
                "Error in function value_ret_func() due to an initial value being zero."
            )
        else:
            if logret:
    ...
    def make_portfolio(self, name: str) -> pd.DataFrame:
        ...
        if self.weights is None:
            raise Exception(
                "OpenFrame weights property must be provided to run the make_portfolio method."
            )
        df = self.tsdf.copy()
        if not any(
            [
